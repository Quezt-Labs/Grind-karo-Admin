...
import { Eye, MessageCircle, UserCheck } from "lucide-react";
...
import {
  buildCoachingClientChatRoute,
  buildCoachingClientFormCheckRoute,
  buildCoachingClientProfileRoute,
} from "@/utils/formCheckRoutes";
...
export function FormCheckCoachingClientsPage() {
  ...
  const clientsById = useMemo(() => {
    const map = new Map<string, FormCheckCoachingClient>();
  ...
  const openFormChecks = useCallback(
    (userId: string) => {
      const client = clientsById.get(userId);
      if (!client) return;
      navigate(buildCoachingClientFormCheckRoute(client));
    },
    [clientsById, navigate],
  );

  const openProfile = useCallback(
    (userId: string) => {
      navigate(buildCoachingClientProfileRoute(userId, isAdmin));
    },
    [isAdmin, navigate],
  );
  ...
  const openChat = useCallback(
    (userId: string) => {
      navigate(buildCoachingClientChatRoute(userId));
    },
    [navigate],
  );
    ...
    <div className="space-y-6">
      ...
      {(isLoading || rows.length > 0) && (
        <DataTable
          ...
          columns={columns}
          isLoading={isLoading}
          onRowClick={(row) => openFormChecks(row.id)}
        />
      )}
    </div>
  );
